...
Usage:
    ...
    python evol_instruct/scripts/10_generate_qa_pairs.py --max-docs 50 --dry-run
    python evol_instruct/scripts/10_generate_qa_pairs.py --restart          # bỏ checkpoint cũ
...
không có tác dụng:
    python evol_instruct/scripts/10_generate_qa_pairs.py --dry-run --rebuild-corpus
    python evol_instruct/scripts/10_generate_qa_pairs.py --source data/corpus_khac --rebuild-corpus
"""
...
from evol_instruct.src.qa_specificity.pair_generator import generate_pair
from evol_instruct.src.utils import Checkpoint, append_jsonl, setup_logging
...
def main():
    ...
    logger.info("   Bước tiếp theo: python evol_instruct/scripts/11_verify_labels.py")

...
Usage:
    python evol_instruct/scripts/11_verify_labels.py --input data/qa_pairs/raw/pairs.jsonl
    python evol_instruct/scripts/11_verify_labels.py --estimate-cost     # chỉ ước lượng chi phí judge, không gọi API
    python evol_instruct/scripts/11_verify_labels.py --no-judge          # chỉ chạy heuristic, không tốn API
    python evol_instruct/scripts/11_verify_labels.py --strict-consensus  # heuristic ambiguous = bất đồng
    python evol_instruct/scripts/11_verify_labels.py --export-manual-sample 100
    python evol_instruct/scripts/11_verify_labels.py --compute-kappa data/qa_pairs/labeled/manual_sample_blind.jsonl
"""
...
from evol_instruct.src.utils import load_jsonl, save_jsonl, setup_logging
...
def cmd_export_manual_sample(args, logger):
    ...
    logger.info(f"      python evol_instruct/scripts/11_verify_labels.py --compute-kappa {args.manual_sample}")
...
def cmd_verify(args, logger):
    ...
    if pass_rate > PASS_RATE_GOOD:
        ...
        logger.info(f"   python evol_instruct/scripts/11_verify_labels.py --export-manual-sample {MANUAL_SAMPLE_SIZE}")
        logger.info("   python evol_instruct/scripts/12_export_dataset.py")

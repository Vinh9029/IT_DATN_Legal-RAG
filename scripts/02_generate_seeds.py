...
Usage:
    python scripts/02_generate_seeds.py --mode gemini --target-seeds 1000
    python scripts/02_generate_seeds.py --mode template --target-seeds 1000
    python scripts/02_generate_seeds.py --mode both --target-seeds 1000 --max-docs 400
"""
...
from src.utils import setup_logging, save_jsonl, generate_item_id
...
from scripts.seed_gen_helpers import generate_seeds_with_gemini, generate_seeds_with_llm_client
...
Examples:
  ...
  # Kết hợp cả hai
  python scripts/02_generate_seeds.py --mode both --target-seeds 1000
        """,
    )
    parser.add_argument(
        "--mode",
    ...
    log.info(f"   Output: data/seeds/seeds.jsonl")
    log.info("=" * 60)
